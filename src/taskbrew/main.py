...
TASKBREW_DIR = Path.home() / ".taskbrew"
PID_FILE = TASKBREW_DIR / "taskbrew.pid"
LOG_FILE = TASKBREW_DIR / "taskbrew.log"
...
def _validate_startup(project_dir: Path, team_config, roles: dict, cli_provider: str):
    ...
    if not roles:
        logging.getLogger(__name__).warning(
            "No role files found in config/roles/ — no agents will run. "
            "Add role YAML files or run: taskbrew init"
        )
...
def _cmd_start(args):
    """Start TaskBrew as a background daemon."""
    existing_pid = _read_pid()
    if existing_pid:
        print(f"TaskBrew is already running (PID: {existing_pid}).")
        print("Run `taskbrew logs` to view logs, `taskbrew stop` to stop.")
        return

    TASKBREW_DIR.mkdir(parents=True, exist_ok=True)

    # Spawn the foreground `serve` process in the background.
    # stdout/stderr → devnull; all logging goes through the file handler.
    cmd = [sys.executable, "-m", "taskbrew.main", "--_serve_foreground"]
...
def _cmd_stop(_args):
    """Stop the background TaskBrew daemon."""
    pid = _read_pid()
    if pid is None:
        print("TaskBrew is not running.")
        return

    print(f"Stopping TaskBrew (PID: {pid})...")
    try:
        os.kill(pid, signal.SIGTERM)
    except ProcessLookupError:
        _remove_pid()
        print("Process already exited. Cleaned up PID file.")
        return
    ...
    _remove_pid()
    print("TaskBrew stopped.")


def _cmd_logs(args):
    """Tail the TaskBrew log file."""
    if not LOG_FILE.exists():
        print(f"No log file found at {LOG_FILE}")
        print("Is TaskBrew running? Start it with `taskbrew start`.")
        return

    pid = _read_pid()
    if pid is None:
        print("Note: TaskBrew daemon is not running. Showing existing log file.\n")
    ...
    if not getattr(args, "follow", True):
        return

    # Follow mode — keep tailing
...
def cli_main():
    ...
    # Internal flag: run server in foreground (used by daemon subprocess)
    # Must check before setup_logging() to avoid duplicate log lines.
    if "--_serve_foreground" in sys.argv:
        sys.argv.remove("--_serve_foreground")
        from taskbrew.logging_config import setup_file_logging
        setup_file_logging()
        args = argparse.Namespace(command="serve", project_dir=None)
        # Check for --project-dir in remaining args
    ...
    start_parser = sub.add_parser("start", help="Start the orchestrator server (background)")
    ...
    # serve — foreground mode (backwards compat)
    serve_parser = sub.add_parser("serve", help="Start server in foreground")
    ...
    # logs
    logs_parser = sub.add_parser("logs", help="View server logs")
    logs_parser.add_argument("-n", "--lines", type=int, default=50, help="Number of lines to show")
    logs_parser.add_argument("--no-follow", dest="follow", action="store_false", default=True,
    ...
    if args.command == "init":
        ...
        # No subcommand given — default to background start
        args = argparse.Namespace(command="start", project_dir=None, host=None, port=None)
        _cmd_start(args)
